...
int main(int argc, char** argv)
{
    ...
    uint64_t time_prev_cycle = start_time;

    for(int i = 0; i < LLC_LINE; i++){
        llc_remap_table[i] = -1;
    }

    while (run_simulation) {
        uint64_t time_curr_cycle = time(0);
        if(time_curr_cycle%2 == 0 && time_curr_cycle-time_prev_cycle == 1) remap_cache();           //225 seconds interval for remapping LLC cache
        time_prev_cycle = time_curr_cycle;

        uint64_t elapsed_second = (uint64_t)(time_curr_cycle - start_time),
                 elapsed_minute = elapsed_second / 60,
                 elapsed_hour = elapsed_minute / 60;
        elapsed_minute -= elapsed_hour*60;
        elapsed_second -= (elapsed_hour*3600 + elapsed_minute*60);
...
void remap_cache(){
    ...
    BLOCK** block = llc->block;

    // cout << "remap" << endl;

    int cnt_hot_lines = 0;

    int time_remap = time(0);

    vector<int> sets_hot_cold[llc->NUM_SET][2];
    ...
    //         if(block[i][j].valid == false) continue;
    //         int age = time_remap - block[i][j].alloc_time;
    //         int cnt_hits = block[i][j].cnt_hits;
    ...
    // for(int i = 0; i < llc->NUM_SET; i++){
    //     for(int j = 0; j < llc->NUM_WAY; j++){
    //         cout << block[i][j].address << "\n";
    //     }
    // }

    for(int i = 0; i < llc->NUM_SET; i++){
        for(int j = 0; j < llc->NUM_WAY; j++){
            if(block[i][j].valid == false){
                continue;
            }
            int age = time_remap - block[i][j].alloc_time;
            int cnt_hits = block[i][j].cnt_hits;
            ...
            if(age >= 2){
                if(cnt_hits >= 2){
                    ...
                    cnt_hot_lines++;
                    sets_hot_cold[i][0].push_back(j);
                } else {
                    // cold line

                    sets_hot_cold[i][1].push_back(j);
                }

            }

            block[i][j].cnt_hits = 0;

            // cout << "age " << age << " " << "hits " << cnt_hits << " "<< block[i][j].used <<"\n";
        }
    }
    ...
    int avg_hot_lines = cnt_hot_lines/llc->NUM_SET;

    // cout << "avg_hot_lines " << avg_hot_lines << "\n";

    vector<pair<int,int>> to_remap;
    for(int i = 0; i < llc->NUM_SET; i++){
        // cout << i << " no. of hot " << sets_hot_cold[i][0].size() << "\n";
        // cout << i << " no. of cold " << sets_hot_cold[i][1].size() << "\n";

        
        int extra_hot = sets_hot_cold[i][0].size() - avg_hot_lines;
        for(int k = 0; k < extra_hot; k++){
            to_remap.push_back({i,sets_hot_cold[i][0][k]});
        }

        int remap_cnt = min(-extra_hot, (int)min(to_remap.size(),sets_hot_cold[i][1].size()));
        // cout << remap_cnt << "\n";
        ...
        for(int k = 0; k < remap_cnt; k++){
            pair<int,int> line_remap = to_remap.back();

            //(uint32_t) (address & ((1 << lg2(NUM_SET)) - 1))

//            cout << "address " << " " << bitset<64>(block[line_remap.first][line_remap.second].address) << " " << ((block[line_remap.first][line_remap.second].address)&(uint64_t)(1<<15-1))<< "\n";

//            cout << "address " << " " << bitset<64>(block[i][sets_hot_cold[i][1][k]].address) << " " << ((block[i][sets_hot_cold[i][1][k]].address)&(uint64_t)(1<<15-1))<< "\n";

            llc_remap_table[(uint32_t) (block[line_remap.first][line_remap.second].address & ((1 << lg2(LLC_LINE)) - 1))] = i;
            llc_remap_table[(uint32_t) (block[i][sets_hot_cold[i][1][k]].address & ((1 << lg2(LLC_LINE)) - 1))] = line_remap.first;

            swap(block[line_remap.first][line_remap.second],block[i][sets_hot_cold[i][1][k]]);
            swap(block[line_remap.first][line_remap.second].lru,block[i][sets_hot_cold[i][1][k]].lru);



            //cout << "hot line addr in table " << (line_remap.first*LLC_WAY + line_remap.second) % LLC_LINE << "\n";
    ...
    // for(int i = 0; i < LLC_LINE; i++){
    //     cout << i << " mapped to " << llc_remap_table[i] << "\n";
    // }
}