		...
		if(mshr_index == -2)
		  ...
		  if (cache_type == IS_L1I)
			...
			cpu = 0;
                    ...
                    fill_cache(set, way, &WQ.entry[index]);           //replace way
...
{
    if(cache_type == IS_LLC){
      int remapped_set = llc_remap_table[(uint32_t) (address & ((1 << lg2(LLC_LINE)) - 1))];
      return (remapped_set == -1 ? (uint32_t) (address & ((1 << lg2(NUM_SET)) - 1)) : remapped_set); 
    } else {
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    if(cache_type == IS_LLC){
      block[set][way].alloc_time = time(0);             //alloc_time
      block[set][way].cnt_hits = 1;                     //cnt_hits

      llc_remap_table[(uint32_t) (block[set][way].address & ((1 << lg2(LLC_LINE)) - 1))] = -1;

//      cout << set << " " << way << " fill" << endl;
    }
    ...
    if(cache_type == IS_LLC) llc_remap_table[(uint32_t) (block[set][way].address & ((1 << lg2(LLC_LINE)) - 1))] = -1;

    // cout << "address "<< bitset<64>(block[set][way].address) << "\n";
    // cout << "full_addr "<< bitset<64>(block[set][way].address)  << "\n";
...
int CACHE::check_hit(PACKET *packet)
{
    ...
    // hit
    for (uint32_t way=0; way<NUM_WAY; way++) {
        if (block[set][way].valid && (block[set][way].tag == packet->address)) {      //block is cache storage, i.e 2D array, row = way = BLOCK
//            cout << "HIT"<< endl;
            if(cache_type == IS_LLC){ /*cout << "HIT1"<< endl;*/ block[set][way].cnt_hits++; /*cout << "HIT2"<< endl;*/ }
